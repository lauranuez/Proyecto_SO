...
namespace v1
{
    public partial class Inicio : Form
    {
        ...
        public void QuitaNoitficacion(string a) //Metodo para quitar la notificacion del label
        {
            try
            {
                this.accept_invitation.Text = "No tienes invitaciones";
                this.accept_invitation.BackColor = Color.Transparent;
            }
            catch (FormatException)
            {
                MessageBox.Show("No se han podido mostrar correctamente los datos");
        ...
        private void enviar_Btn_Click(object sender, EventArgs e) //Metodo para enviar al servidor el mensaje de registrarse con los datos de ese usuario
        {
            ...
            if (er == 0)
            {
                try
                {
                    string mensaje = "2/" + usuarioRe + "/" + contraseñaRe + "/" + edad;
                    byte[] msg = System.Text.Encoding.ASCII.GetBytes(mensaje);
                    server.Send(msg);
                }
                catch (FormatException)
        ...
        private void deny_Click(object sender, EventArgs e) //Metodo para enviar al servidor que el jugador a rechazado la partida
        {
            try
            {
                ...
                byte[] msg = System.Text.Encoding.ASCII.GetBytes(mensaje);
                server.Send(msg);
                this.Invoke(new DelegadoParaPonerTexto(QuitaNoitficacion), new Object[] { accept_invitation.Text });
